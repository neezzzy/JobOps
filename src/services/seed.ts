...
export async function seedDemoData() {
  ...
  for (const sample of samples) {
    ...
    await createApplication({
      ...
      priority: sample.status === 'Interview' ? 'High' : 'Normal',
      archived_at: null,
      next_action_type: sample.status === 'Saved' ? 'Apply by' : sample.status === 'Applied' ? 'Follow up' : sample.status === 'Interview' ? 'Prepare' : null,
      next_action_date: sample.follow_up_date,
